...
class FlaskSlackbot(object):
    def __init__(self, slackbot_conf, flask_port=8194, slack_port=8193,
        ...
        Inputs:
        - slackbot_conf is a loaded yaml that was generated by create_slackbot_config.py
    ...
    def send_scheduled_messages(self):
        """
        This first deletes every single message that this bot has scheduled that
        is more than 5 minutes in the future. It then schedules every message
        that has not yet been scheduled. Specifically, the pre-study message,
        the daily intro messages, and the end-of-day messages are all pre-scheduled.
        Any messages with images are not pre-scheduled, since the Slackbot has
        to first get images from the robot before sending them.
        """
    ...
    def body_to_question_answer(self, body):
        ...
        TODO: this can be hardcoded once we fix a particular template message
        TODO: this is non-ideal style, TBH I should use a try/except block to
    ...
    def send_end_of_day_message(self, user_id, day, is_last_day, survey_i, timestamp=None):
        """
        Send the end-of-day message to user_id. If timestamp is None or has
        passed, send immediately. Else, schedule.
        """
    ...
    def send_daily_intro_message(self, user_id, day, timestamp=None):
        """
        Send the daily intro message to user_id. If timestamp is None or has
        passed, send immediately. Else, schedule.
        """
        if user_id in self.users:
    ...
    def send_pre_study_message(self, user_id, timestamp=None):
        """
        Send the pre-study message, If timestamp is None, send immediately.
        Else, schedule.
        """
        if user_id in self.users:
...
if __name__ == '__main__':
    ...
    if is_deployed:
        ...
        log_filepath = "../logs/log_debug.txt"
    ...
    if is_deployed:
        ...
        slackbot_conf_filepath = "../cfg/slackbot_debug.yaml"
